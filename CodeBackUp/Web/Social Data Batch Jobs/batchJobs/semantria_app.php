...
class SemantriaApp extends BatchAdmin
{
    ...
    function BuildSentimentAnalysysData()
    {
		...
		$bp_sa_recs = $this->m_semantria_persistence->GetBpSentimentAnalysysData();
        $analizer = new SemantriaAnalyser();
        print_r($bp_sa_recs);		
        if(count($bp_sa_recs) > 0){
	        $analyzedArray =  $analizer->analyzeData($bp_sa_recs);
	        print_r($analyzedArray);
	        if(count($analyzedArray) >0 ){
	        	$this->m_semantria_persistence->UpdateBPAnalyzedData($analyzedArray);
	        	$this->m_semantria_persistence->UpdateTHEMESAnalyzedData($analyzedArray);
	        	$this->m_semantria_persistence->UpdateENTITIESAnalyzedData($analyzedArray);
				$this->m_semantria_persistence->UpdatePHRASESAnalyzedData($analyzedArray);
	        }
		}
